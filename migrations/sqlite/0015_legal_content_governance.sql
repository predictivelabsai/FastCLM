...
UPDATE assistant_messages AS current
SET sequence_number = (
    SELECT COUNT(*) FROM assistant_messages AS prior
    WHERE prior.thread_id=current.thread_id AND prior.rowid<=current.rowid
);
